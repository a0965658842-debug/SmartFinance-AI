...
export class GeminiService {
  ...
  static async getFinancialAdvice(
    ...
    try {
      ...
      // Fix: Using systemInstruction for role setting as per guidelines
  ...
  static async getDailyFortune(): Promise<{ title: string; poem: string; meaning: string; luck: string }> {
    ...
    try {
      // Fix: Improved prompt structure and used systemInstruction
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `目前的主題關鍵字是：${randomTheme}，隨機標識：${seed}`,
        config: { 
          systemInstruction: `為使用者生成一張繁體中文的財運詩籤。內容必須完全原創，每次都要有不同的意境。包含一個有古風味道的標題（如：第七七籤 鳳凰來儀）。四句詩要押韻且優美。解析要貼近現代生活中的金錢與理財觀念。幸運指數用星星符號 (⭐) 表示，1到5顆星。`,
          ...
          responseSchema: {
            ...
            propertyOrdering: ["title", "poem", "meaning", "luck"],
          }
        }
      });
